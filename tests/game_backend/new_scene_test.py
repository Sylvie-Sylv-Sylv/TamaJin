...
class Health:
    schema = np.dtype([("value", np.int32)], align=True)
...
# create entities
scene.add_entity(
    "player", {Position: (10.0, 20.0), Velocity: (1.5, 0.25), Health: (100,)}
)

scene.add_entity(
    "enemy_1", {Position: (50.0, 80.0), Velocity: (-0.5, 0.0), Health: (40,)}
)

scene.add_entity(
    "enemy_2", {Position: (100.0, 120.0), Velocity: (-1.0, 0.5), Health: (60,)}
)

# movement system
for entity, pos, vel in scene.query(Position, Velocity):
    pos["x"] += vel["x"]
    pos["y"] += vel["y"]

    print(f"{entity} moved to " f"({pos['x']:.2f}, {pos['y']:.2f})")
...
for entity, health in scene.query(Health):
    ...
    print(f"{entity} health = " f"{health['value']}")

print()

# fetch single component
player_pos = scene.fetch_component("player", Position)

print("player final position:", player_pos["x"], player_pos["y"])
