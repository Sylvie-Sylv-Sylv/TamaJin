...
class RenderSystem(System):
    ...
    def step(self, scene, **kwargs):
        ...
        for y in range(0, 600, 40):
            pygame.draw.rect(self.surface, (255, 255, 255), (398, y, 4, 20))
        ...
        for entity in ("left_paddle", "right_paddle"):
            ...
            rect = pygame.Rect(int(pos["x"] - 10), int(pos["y"] - 50), 20, 100)
            pygame.draw.rect(self.surface, (255, 255, 255), rect)
        # Ball
        ball = scene.fetch_component("ball", Position)
        if ball is not None:
            pygame.draw.circle(
                self.surface, (255, 255, 255), (int(ball["x"]), int(ball["y"])), 10
            )
