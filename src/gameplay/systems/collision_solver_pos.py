...
class CollisionSolverPos(System):
    ...
    def step(scene, dt):
        for entity_a, entity_b in scene.narrow_collision_pairs:
            ...
            if (mass_a := scene.fetch(entity_a, Mass)) is not None and (
                mass_b := scene.fetch(entity_b, Mass)
            ) is not None:
                total_inv = mass_a["inv"] + mass_b["inv"]

                if (position_a := scene.fetch(entity_a, Position)) is not None:
                    position_a["x"] += mtv[0] * mass_a["inv"] / total_inv * 0.999
                    position_a["y"] += mtv[1] * mass_a["inv"] / total_inv * 0.999
                if (position_b := scene.fetch(entity_b, Position)) is not None:
                    position_b["x"] -= mtv[0] * mass_b["inv"] / total_inv * 0.999
                    position_b["y"] -= mtv[1] * mass_b["inv"] / total_inv * 0.999
