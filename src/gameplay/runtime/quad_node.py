...
class QuadNode:
    ...
    def subdivide(self):
        ...
        self.children = [
            # NW
            QuadNode(AABB(x, y, w, h), self.depth + 1),
            # NE
            QuadNode(AABB(x + w, y, w, h), self.depth + 1),
            # SW
            QuadNode(AABB(x, y + h, w, h), self.depth + 1),
            # SE
            QuadNode(AABB(x + w, y + h, w, h), self.depth + 1),
        ]

    def get_child_for(self, aabb: AABB):
    ...
    def insert(self, entity_id: str, aabb: AABB):
        ...
        self.entities.append((entity_id, aabb))
        ...
        if len(self.entities) > self.MAX_OBJECTS and self.depth < self.MAX_DEPTH:
            ...
            while i < len(self.entities):
                ...
                if child is not None:
                    child.insert(entity, entity_aabb)
                    self.entities.pop(i)
                else:
                    i += 1

    def query(self, area: AABB, found=None):
    ...
    def pg_render(self, surface: pygame.Surface, width: int = 1):
        color = (min(255, self.depth * 40), 255 - min(255, self.depth * 30), 255)
