...
class SharedChunkBuffer:
    ...
    def register_components(self, component_layouts: dict[type, np.dtype[Any]]) -> None:
        total_bytes = 0
        ...
        for component_type, dtype in component_layouts.items():
            ...
            if total_bytes % alignment != 0:
                total_bytes += alignment - (total_bytes % alignment)
        ...
        for component_type, dtype in component_layouts.items():
            ...
            typed_view: NDArray[Any] = np.ndarray(
                shape=(self.capacity,), dtype=dtype, buffer=self.buffer, offset=offset
            )

            self.component_arrays[component_type] = typed_view

    def add_entity(self, entity_id: str, components: dict[type, object]) -> None:
    ...
    def fetch(self, entity_id: str, component_type: type[T]) -> T | None:
    ...
    def query(self, *component_types: type) -> Iterator[tuple[Any, ...]]:
        arrays = [
            self.component_arrays[component_type] for component_type in component_types
        ]

        for i in range(self.size):
            yield (self.index_to_entity[i], *(array[i] for array in arrays))
...
class Scene:
    def __init__(self, name: str, capacity: int = 1024) -> None:
        ...
        self.chunk: SharedChunkBuffer = SharedChunkBuffer(capacity)

        self.pending_components: dict[type, np.dtype[Any]] = {}
    ...
    def register_component(self, component_type: type) -> None:
        if self.initialized:
            raise RuntimeError("Cannot register components after initialization.")

        self.pending_components[component_type] = np.dtype(component_type.schema)

    def register_components(self, component_types: list[type]) -> None:
    ...
    def register_systems(self, systems: list[System]) -> None:
        for system in systems:
            self.register_system(system)

    def initialize(self) -> None:
        self.chunk.register_components(self.pending_components)

        self.initialized = True

    def add_entity(self, entity_id: str, components: dict[type, object]) -> None:
        if not self.initialized:
            raise RuntimeError("Scene.initialize() must be called first.")

        self.entities.add(entity_id)

        self.chunk.add_entity(entity_id, components)
    ...
    def fetch_component(self, entity_id: str, component_type: type[T]) -> T | None:
        return self.chunk.fetch(entity_id, component_type)

    def fetch_system(self, system_type: type[System]) -> System | None:
        return self.systems.get(system_type.__name__)

    def query(self, *component_types: type) -> Iterator[tuple[Any, ...]]:
        yield from self.chunk.query(*component_types)
